...
export default function VistaAnoServicio({ publicadores }) {
  ...
  const debiaInformarEnMes = (publicador, mes, ano) => {
    ...
    const fechaBase =
      publicador.informar_desde ||
      publicador.en_congregacion_desde ||
      publicador.activo_desde
  ...
  const loadAnalisis = async () => {
    ...
    try {
      // IMPORTANTE:
      // El año de servicio se determina a partir del ÚLTIMO MES VENCIDO.
      //
      // Ejemplo:
      // 01/09/2026 → último mes vencido = agosto 2026
      //             → año de servicio = 2025-2026
      //
      // 01/10/2026 → último mes vencido = septiembre 2026
      //             → año de servicio = 2026-2027
      const anoServicio = getAnoServicioVencido()
      const mesesAnoServicio = getMesesAnoServicio(anoServicio)

      // Último mes vencido
      const { mes: mesVencido, ano: anoVencido } = getMesVencido()
      ...
      //
      // Se considera cerrado un mes anterior al último mes vencido.
      // El propio mes vencido se considera cerrado a partir del día 6,
      // siguiendo el criterio utilizado anteriormente.
      const mesesCerradosSet = new Set(
        mesesAnoServicio
          .filter(m => {
            const fechaMes = new Date(m.ano, m.mes - 1, 1)
            const fechaVencido = new Date(
              anoVencido,
              mesVencido - 1,
              1
            )
      ...
      const precursores = publicadores.filter(
        p =>
      ...
      // Traer los 12 meses del año de servicio correspondiente
      ...
      for (const pub of precursores) {
        ...
        for (const mesInfo of mesesAnoServicio) {
          ...
          const informe = informes.find(
            i => String(i.publicador_id) === String(pub.id)
          )
        ...
        // Posición del último mes vencido dentro del año de servicio
        //
        // 1  = Septiembre
        // 2  = Octubre
        // ...
        ...
        if (mesVencido >= 9) {
          mesDelAnoServicio = mesVencido - 8
        } else {
          mesDelAnoServicio = mesVencido + 4
        }

        const totalMesesAno = 12

        // IMPORTANTE:
        // Cuando agosto vence, ya no queda ningún mes por delante.
        //
        // Ejemplo:
        // Agosto = mes 12
        // 12 - 12 = 0 meses restantes
        const mesesRestantes =
          totalMesesAno - mesDelAnoServicio
...
const proyeccion12Meses =
  promedioParaProyeccion * 12
        ...
        const horasFaltanPara600 = Math.max(
          600 - horasTotales,
          0
        )

        const horasMesPara560 =
          mesesRestantes > 0
            ? horasFaltanPara560 / mesesRestantes
            : 0

        const horasMesPara600 =
          mesesRestantes > 0
...
if (mesesRestantes === 0) {
  ...
  estado
      ...
      analisisPrecursores.sort((a, b) => {
        ...
        if (
          estadoOrder[a.estado] !==
          estadoOrder[b.estado]
        ) {
          return (
            estadoOrder[a.estado] -
            estadoOrder[b.estado]
  ...
  // El encabezado utiliza exactamente el mismo año de servicio
  ...
  const stats = {
    ...
    atencion: analisis.filter(
      a => a.estado === 'atencion'
    ).length,

    bajoMeta: analisis.filter(
      a => a.estado === 'bajo-meta'
    ).length,

    enMeta: analisis.filter(
      a => a.estado === 'en-meta'
    ).length,
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        {analisisFiltrado.map(prec => {
          ...
          const faltanPara560 = Math.max(
            560 - prec.horasTotales,
            0
          )

          const faltanPara600 = Math.max(
            600 - prec.horasTotales,
            0
          )
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-600 mt-1">
                    Grupo {prec.grupo} •{' '}
                    {prec.mesesInformados}/
                    {prec.totalMesesAno} meses informados
                  </div>
                </div>
        ...
        {analisisFiltrado.length === 0 && (
          <div className="card p-12 text-center">
            <p className="text-slate-600">
              No hay precursores en esta categoría
            </p>
          </div>
        )}
