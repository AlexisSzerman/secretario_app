...
export default function VistaInformeS1({ publicadores, informes, mesActual }) {
  ...
  const [publicadoresIrregulares, setPublicadoresIrregulares] = useState([])
  ...
  // FUNCIÓN HELPER: Verifica si debía informar en un mes/año dado
  const debiaInformarEnMes = (publicador, mes, ano) => {
    ...
    if (publicador.fecha_mudanza) {
      ...
      const fechaMesCheck = new Date(ano, mes - 1, 1)
      if (fechaMesCheck > fechaSalida) return false
    }
  ...
  // Versión sin parámetros (para el mes actual del informe)
  const debiaInformarEnMesActual = (publicador) => {
    return debiaInformarEnMes(publicador, mesActual.mes, mesActual.ano)
  }
  ...
  const publicadoresDeberian = publicadores.filter(p => debiaInformarEnMesActual(p))
  ...
  // 5. Irregulares y alerta (lógica del Dashboard: 3 meses cerrados anteriores)
  useEffect(() => {
    calcularAlerta()
  }, [publicadores, informes, mesActual])

  const calcularAlerta = async () => {
    const mesActual_num = mesActual.mes
    const anoActual_num = mesActual.ano

    // Buscamos 5 meses atrás para asegurar obtener 3 válidos
    // (algún mes puede quedar null si debiaInformarEnMes lo excluye)
    const mesesCerrados = []
    for (let j = 1; j <= 5; j++) {
      let mes = mesActual_num - j
      let ano = anoActual_num
    ...
    for (const pub of publicadoresDeberian) {
      const participaciones = await Promise.all(
        mesesCerrados.map(async ({ mes, ano }) => {
          if (!debiaInformarEnMes(pub, mes, ano)) return null
          try {
            const informesMes = await db.getInformesByMesAno(mes, ano)
            const inf = informesMes.find(i => i.publicador_id === pub.id)
            return inf?.participo || false
          } catch {
            return null
          }
        })
      ...
      // Tomar solo los primeros 3 meses válidos (con null excluidos)
      const validas = participaciones.filter(p => p !== null).slice(0, 3)

      if (validas.length >= 3 && validas.every(p => p === false)) {
        irregulares.push(pub)
      } else if (validas.length === 2 && validas.every(p => p === false)) {
        alerta.push(pub)
      }
    }

    setPublicadoresIrregulares(irregulares)
  ...
  const exportarPDF = () => {
    ...
    if (publicadoresIrregulares.length > 0)
      allLists.push({ title: 'IRREGULARES',            items: publicadoresIrregulares,     bg: [254,226,226], hd: [153,27,27] })
    if (publicadoresAlerta.length > 0)
      allLists.push({ title: 'IRREGULARES PROXIMO MES',items: publicadoresAlerta,          bg: [255,237,213], hd: [154,52,18] })
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {publicadoresIrregulares.length > 0 && (
        <div className="card p-6 bg-red-50 border-red-200">
          <h4 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
            <AlertCircle className="text-red-600" size={20} />
            Irregulares ({publicadoresIrregulares.length})
          </h4>
          <p className="text-sm text-red-700 mb-3">
            3 meses consecutivos sin participar en la predicación
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {publicadoresIrregulares.map(pub => (
              <div key={pub.id} className="text-sm text-slate-700">
                • {pub.apellido}, {pub.nombre}
              </div>
            ))}
      ...
      {publicadoresAlerta.length > 0 && (
        <div className="card p-6 bg-orange-50 border-orange-200">
          ...
          <p className="text-sm text-orange-700 mb-3">
            2 meses consecutivos sin participar — serán irregulares si no informan el próximo mes
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
